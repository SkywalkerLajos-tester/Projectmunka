...
class TestRegistration:
    ...
    def test_registration_eng_with_wrong_email_confirm(self):
        ...
        button.click()
        time.sleep(0.5)
        assert self.registration_page.error_confirm_email_missing().is_displayed()
        assert self.registration_page.error_confirm_email_missing().text == "Email does not match"
        assert self.browser.current_url != "http://localhost:4200/"
